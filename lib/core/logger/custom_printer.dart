...
/// Custom printer với format đẹp hơn
/// 
...
class CustomPrinter extends LogPrinter {
  ...
  CustomPrinter({
    ...
    // Translate excludeBox map to includeBox map
    includeBox = {};
  ...
  List<String> log(LogEvent event) {
    ...
    // Format error object thành JSON đẹp thay vì toString()
    String? errorStr;
    if (event.error != null) {
      try {
        if (event.error is Map) {
          errorStr = stringifyMessage(event.error);
        } else {
          // Thử convert thành JSON nếu có toJson()
          try {
            final json = (event.error as dynamic).toJson();
            errorStr = stringifyMessage(json);
          } catch (_) {
            // Nếu không có toJson(), dùng toString() nhưng giới hạn độ dài
            final errorString = event.error.toString();
            // Bỏ qua các error message dài dòng không cần thiết
            if (errorString.length > 500) {
              errorStr = null; // Không hiển thị error object quá dài
            } else {
        ...
        errorStr = null; // Bỏ qua nếu không format được
      }
    }
  ...
  List<String> _formatAndPrint(
    ...
    if (error != null && includeBox[level]!) buffer.add(color(_middleBorder));
